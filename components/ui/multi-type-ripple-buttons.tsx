...
function removeRippleById(ripples: RippleState[], key: number): RippleState[] {
  return ripples.filter(r => r.key !== key);
}

function buildRipple(event: MouseEvent<HTMLButtonElement>, color: string): RippleState {
...
const RippleButton: React.FC<RippleButtonProps> = ({
  ...
  const dynamicGridHoverStyles = useMemo(() => {
    ...
    if (variant === 'hover') {
      const actualHoverRippleColor = customHoverRippleColor ?? hexToRgba(hoverBaseColor, 0.466);
      return `
  ...
  const createJsRipple = (event: MouseEvent<HTMLButtonElement>) => {
    const newRipple = buildRipple(event, determinedJsRippleColor);
    setJsRipples(prev => [...prev, newRipple]);
    setTimeout(() => {
      setJsRipples(current => removeRippleById(current, newRipple.key));
    }, rippleDuration);
  };
  ...
  if (variant === 'hover') {
    const cls = cn(
      "relative rounded-lg text-base px-4 py-2",
    ...
    return (
      <>
        ...
        <button className={cls} onClick={handleButtonClick} disabled={disabled}>
          <span className="relative z-[10] pointer-events-none" style={{ color: 'inherit' }}>{children}</span>
          {jsRippleElements}
          <div
  ...
  if (variant === 'hoverborder') {
    const cls = cn(
      "relative rounded-lg overflow-hidden text-base px-4 py-2",
  ...
  if (variant === 'ghost') {
    const cls = cn(
      "relative border-none bg-transparent isolate overflow-hidden cursor-pointer px-4 py-2 rounded-lg text-base",
      disabled && "opacity-50 cursor-not-allowed pointer-events-none",
      className,
    );
    return (
